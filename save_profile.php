<?php
	if(isset($_COOKIE["login"])){
		...
		if(isset($_REQUEST["id"])){
			...
			$rs=mysqli_query($conn,"select * from interested where (email='$login_email' AND code='$id') OR (email='$user_email' AND code='$code')");
			...
			if($r=mysqli_fetch_array($rs)){
				...
				if($rs=mysqli_query($conn,"insert into interested values('$login_email','$id')")>0){
					echo "success";
				}
				else{
					echo "again";
